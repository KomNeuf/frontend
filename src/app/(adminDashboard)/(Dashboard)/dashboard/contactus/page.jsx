...
export default function Contact() {
  ...
  const {
    ...
    refetch,
    error,
  } = api.adminApis.useGetAllContactsQuery("");

  useEffect(() => {
    if (contact) {
      setAllContacts(contact?.data);
    }
  }, [contact]);
  ...
  ] = api.adminApis.useDeleteContactMutation();
  ...
  const initiateDelete = (item) => {
    setItemToDelete({ id: item?._id, name: `${item?.subject}` });
    setIsDeleteModalOpen(true);
  };
  const handleDelete = (id) => {
    deleteTrigger(id);
    setAllContacts(allContacts.filter((item) => item._id !== id));
  };
  ...
  const { items } = usePagination({
    count: Math.ceil(allContacts?.length / rowsPerPage),
    onChange: (event, page) => handleChange(event, page),
    page: page,
  });

  const filterData = (allContacts, searchQuery) => {
    ...
    <main className="p-3 md:p-3 lg:p-4 xl:p-6">
      ...
      <section className="mt-5">
        <div className="table_container overflow-auto max-w-[1030px] relative max-h-[calc(100vh-210px)]">
          <TableContainer component={Paper}>
            <Table className="w-full" cellPadding={14}>
              ...
              {isFetching ? (
                ...
                <TableBody>
                  {filterData(allContacts, searchData.name)
                    ...
                    .map((each, index) => (
                      ...
                      >
                        ...
                        <TableCell className="gap-1.5 lg:gap-2 xl:gap-2.5 single-line-text">
                          <Link
                            href={`/dashboard/contactus/${each._id}?action=view`}
                          ...
                          <Link
                            href={`/dashboard/contactus/${each._id}?action=update`}
                            className="!inline mx-1"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
          ...
          {!(isLoading || isFetching) && allContacts?.length === 0 && (
