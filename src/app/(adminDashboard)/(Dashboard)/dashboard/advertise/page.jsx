...
export default function Contact() {
  ...
  const [allAdvertises, setAllAdvertises] = useState([]);
  ...
  useEffect(() => {
    if (advertise) {
      setAllAdvertises(advertise?.data);
    }
  }, [advertise]);
  ...
  ] = api.adminApis.useDeleteAdvertisementMutation();
  ...
  const initiateDelete = (item) => {
    setItemToDelete({ id: item?._id, name: `${item?.firstName}` });
    setIsDeleteModalOpen(true);
  };
  const handleDelete = (id) => {
    deleteTrigger(id);
    setAllAdvertises(allAdvertises.filter((item) => item._id !== id));
  };
  ...
  const { items } = usePagination({
    count: Math.ceil(allAdvertises?.length / rowsPerPage),
    onChange: (event, page) => handleChange(event, page),
    page: page,
  });

  const filterData = (allAdvertises, searchQuery) => {
    if (!searchQuery) return allAdvertises;
    return allAdvertises.filter((item) => {
      return (
    ...
    <main className="p-3 md:p-3 lg:p-4 xl:p-6">
            ...
            <label htmlFor="name" className="lg:w-[60%] relative max-h-max">
              <span className="relative">
                ...
                <input
                  ...
                  type="text"
                  placeholder="Search by First Name, Last Name, Phone or Comapany"
      ...
      <section className="mt-5">
        <div className="table_container overflow-auto max-w-[1030px] relative max-h-[calc(100vh-210px)]">
          <TableContainer component={Paper}>
            <Table className="w-full" cellPadding={14}>
              ...
              {isFetching ? (
                ...
                <TableBody>
                  {filterData(allAdvertises, searchData.name)
                    ...
                    .map((each, index) => (
                      ...
                      >
                        ...
                        <TableCell className="text-zinc-600 tracking-wide font-medium single-line-text">
                          {truncateText(each.message, 15)}
                        </TableCell>
                        <TableCell className="gap-1.5 lg:gap-2 xl:gap-2.5 single-line-text">
                          <Link
                            href={`/dashboard/advertise/${each._id}?action=view`}
                          ...
                          <Link
                            href={`/dashboard/advertise/${each._id}?action=update`}
                            className="!inline mx-1"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
          ...
          {!(isLoading || isFetching) && allAdvertises?.length === 0 && (
