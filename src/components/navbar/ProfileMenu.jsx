...
const ProfileMenu = ({ onLogout, loginUser }) => {
  const [isOpen, setIsOpen] = useState(false);
  const menuRef = useRef(null);
  const pathname = usePathname();
  const lang = pathname?.split("/")[1];
  const t = langs[lang]?.profileMenu;
    ...
    <div className="relative" ref={menuRef}>
      ...
      >
        <img
          ...
          className="relative inline-block object-cover border border-primaryText object-center w-10 h-10 rounded-full "
        />
      ...
      {isOpen && (
        <ul
          role="menu"
          className="absolute z-40 top-12  right-0 flex min-w-[160px] flex-col gap-2 overflow-auto rounded-md border border-blue-gray-50 bg-primaryGray p-3 font-sans text-sm font-normal primaryText shadow-lg shadow-blue-gray-500/10 focus:outline-none "
        >
          <Link
          ...
          >
            ...
            <p className="block font-sans text-sm  font-medium leading-normal text-inherit">
              {t?.profile}
            </p>
          </Link>
          <Link
          ...
          <Link
            ...
            className="flex w-full cursor-pointer select-none items-center gap-2 rounded-md px-3 pt-[9px] pb-2 text-start leading-tight outline-none transition-all hover:bg-primaryBg "
          >
            <ListOrdered className="text-[#90A4AE] w-4 h-4" />
            <p className="block font-sans text-sm antialiased font-medium leading-normal text-inherit">
              {t?.myOrders}
            </p>
          </Link>

          <hr className="my-1 border-primaryBg" role="menuitem" />
          <button
            role="menuitem"
            onClick={onLogout}
            className="flex w-full cursor-pointer select-none items-center gap-2 rounded-md px-3 pt-[9px] pb-2 text-start leading-tight outline-none transition-all hover:bg-primaryBg "
